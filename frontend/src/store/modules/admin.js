...
const formatDataListByMonth = function (data) {
  ...
  let listData = data
  for (let i = 0; i < 12; i++) {
    let listItem = listData.filter((item) => new Date(item.createdAt).getMonth() == i)
...
const mutations = {
  ...
  setRegisteredUserMonth(state) {
    state.numberOfRegisteredUsersThisMonth = state.userListMonth[new Date().getMonth()]
    state.numberOfRegisteredUsersLastMonth = state.userListMonth[new Date().getMonth() - 1]
  },
  setRegisteredProjectMonth(state) {
    state.numberOfRegisteredProjectsThisMonth = state.projectList[new Date().getMonth()]
    state.numberOfRegisteredProjectsLastMonth = state.projectList[new Date().getMonth()-1]
  },
  setRegisteredUserDate(state, data) {
    let list = data.filter((user) => user.createdAt.split('T')[0] == new Date().toISOString().split('T')[0])
    state.numberOfRegisteredUsersToday = list.length
    let list2 = data.filter((user) => user.createdAt.split('T')[0] == (new Date(new Date().getTime() - 24*60*60*1000)).toISOString().split('T')[0])
